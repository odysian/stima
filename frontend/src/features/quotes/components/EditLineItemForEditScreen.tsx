...
export function EditLineItemForEditScreen(): React.ReactElement | null {
  const navigate = useNavigate();
  const { id, lineItemIndex } = useParams<{ id: string; lineItemIndex: string }>();
  const { draft, updateLineItem, removeLineItem } = useQuoteEdit();
  ...
  const hasMatchingDraft = !!draft && !!id && draft.quoteId === id;

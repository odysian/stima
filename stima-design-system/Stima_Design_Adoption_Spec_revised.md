...
- No hover-scale, parallax, or route-specific motion. **Exception:** a single global cross-fade page transition (≤200ms, `prefers-reduced-motion` respected) is product-approved and scoped to PR-P ([#517](https://github.com/odysian/stima/issues/517)) — see that task's non-goals carve-out.
- No icon-system swap.
...
| 2   | Review-screen primary CTA | Keep exact label in repo; do not adopt mock *"Generate Quote"* unless product approves.                                                             | High       | Copy is product scope.                   | **No-block. Resolved by PR2 ([#489](https://github.com/odysian/stima/issues/489))** — Review shipped with repo copy preserved. |
| 3   | Preview action set        | Ship only actions that exist today; do not add *Send to customer* / *Invoice* if absent.                                                            | High       | Prevents scope creep.                    | **No-block. Resolved by PR3 ([#491](https://github.com/odysian/stima/issues/491))** — Preview shipped with repo's action set. |
...
| 7   | Onboarding steps          | **Confirmed:** 1 step — single window captures `business_name`, `first_name`, `last_name`, `trade_type`, then navigates to `/`. Visuals only; preserve shape.                                                                                                              | High     | Flow integrity confirmed by product.                          | **No-block. Resolved for PR-C ([#511](https://github.com/odysian/stima/issues/511)).**             |
...
## Post-adoption: what agents must read

This spec is the **historical decision record** for the design-adoption initiative. The **operational successor** is [`frontend/UI_SYSTEM.md`](../frontend/UI_SYSTEM.md). It is the living contract for tokens, primitives, composition rules, banned patterns, and acceptance gates.

Agents must open `frontend/UI_SYSTEM.md` for any `area:frontend` work touching visuals. The spec remains useful for understanding why certain decisions were made, but `UI_SYSTEM.md` is the ground truth for day-to-day implementation.

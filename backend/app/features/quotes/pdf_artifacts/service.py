"""Quote authenticated PDF artifact lifecycle coordination."""
...
class QuotePdfArtifactRepositoryProtocol(Protocol):
    """Repository behavior required by the quote PDF artifact slice."""
...
class QuotePdfArtifactService:
    """Own authenticated quote PDF artifact generation and retrieval behavior."""

    def __init__(
        self,
        *,
        repository: QuotePdfArtifactRepositoryProtocol,
    ...
    async def get_pdf_artifact(self, *, user_id: UUID, quote_id: UUID) -> tuple[str, bytes]:
    ...
    async def delete_obsolete_artifact(self, object_path: str | None) -> None:
        """Best-effort cleanup for obsolete quote PDF artifact storage objects."""

"""Invoice email delivery orchestration."""
...
class InvoiceEmailRepositoryProtocol(Protocol):
    """Structural protocol for invoice email delivery persistence needs."""
...
class InvoiceEmailTemplateContext:
    """Template context for the transactional invoice email."""
...
class InvoiceEmailDeliveryService:
    """Send invoice delivery emails while preserving invoice share-flow contracts."""
    ...
    async def _send_with_context(self, context: InvoiceEmailContext) -> None:
        ...
        rendered_context = _build_template_context(
            context=context,
            invoice_pdf_url=f"{self._frontend_url}/share/{share_token}",
        )
        ...
        try:
            await self._repository.persist_invoice_event(
            ...
            LOGGER.warning(
                "invoice email sent without persisted throttle state",
                extra={
                    "invoice_id": str(context.invoice_id),
    ...
    def _render_html(self, context: InvoiceEmailTemplateContext) -> str:
        template = self._template_environment.get_template("invoice_email.html")
...
def _validate_customer_email(customer_email: str | None) -> str:
    ...
    if not normalized_email:
        raise QuoteServiceError(
            detail="Add a customer email before sending this invoice.",
            status_code=422,
        )
...
def _remember_fallback_email_sent_at(context: InvoiceEmailContext) -> None:
    _EMAIL_SENT_FALLBACK_TIMESTAMPS[(context.user_id, context.invoice_id)] = datetime.now(UTC)


def _get_fallback_email_sent_at(context: InvoiceEmailContext) -> datetime | None:
    cache_key = (context.user_id, context.invoice_id)
...
def _build_template_context(
    *,
    ...
    return InvoiceEmailTemplateContext(
        ...
        due_date=_format_due_date(context.due_date),
        invoice_pdf_url=invoice_pdf_url,
    )


def _resolve_business_name(context: InvoiceEmailContext) -> str:
...
def _resolve_contractor_name(context: InvoiceEmailContext, fallback: str) -> str:
    name_parts = [
        part
        for part in (
...
def _build_email_subject(context: InvoiceEmailTemplateContext) -> str:
    return f"Invoice {context.doc_number} from {context.business_name}"


def _render_text(context: InvoiceEmailTemplateContext) -> str:
    lines = [
        f"{context.business_name} sent you an invoice.",
        f"Invoice {context.doc_number}: {context.total_amount}",
    ]
    if context.title:
    ...
    lines.extend(
        [
            "",
            f"View invoice PDF: {context.invoice_pdf_url}",
            "",
